% initial testing to see see how model trained on all data performs on each
%   image individually
clc; clear;
...
model = 'merged';
summary = struct();
for m_idx = 1:length(all_images)

    test_im = all_images{m_idx};
    fprintf('Testing Merged Model on %s Image\n',test_im);
    ...
    for kk = 1:90
        ...
        [all_lambda_ml, recon_ml, recon_const, recon_rand, count] = Reconstruction_Testing_(AlgorithmSettings,ProblemSetup, model, 1);
    ...
    summary.(strcat(model,'_',test_im)) = stats;
    ...
    xlabel('Iteration');
    title(sprintf('Mean, Variability, and Sample Count: %s with Merged Model', test_im));
